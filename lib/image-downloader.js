...
class PromisseHandle {
  ...
  _requestCallback(error, response, body) {
    ...
    if (body) {
      ...
      let finalExt = findExt && findExt[0] ? findExt[0].replace(removeParamsReg, '') : mime.extension(headers['content-type'])



      // console.log(finalExt, filename)
      let savename = filenameFinal + `.${finalExt}`;
      if(typeof filename != 'undefined'  && filename.indexOf('.') > -1) {
        savename = filename;
      }
      const finalPath = path.join(dest, savename)
      //---------
